...
class DeepConvNet():
    def __init__(self,
        ...
        for idx, conv_param in enumerate([
            ...
            self.params[
                f"W{idx+1}"] = weight_init_scales[idx] * np.random.randn(
        ...
        self.layers.append(Relu())
        self.layers.append(Pooling(2, 2, 2))
        self.layers.append(
            Convolution(self.params["W3"], self.params["b3"],
                        conv_param_3["stride"], conv_param_3["pad"]))
        self.layers.append(Relu())
        self.layers.append(
            Convolution(self.params["W4"], self.params["b4"],
                        conv_param_4["stride"], conv_param_4["pad"]))
        self.layers.append(Relu())
        self.layers.append(Pooling(2, 2, 2))
        self.layers.append(
            Convolution(self.params["W5"], self.params["b5"],
                        conv_param_5["stride"], conv_param_5["pad"]))
        self.layers.append(Relu())
        self.layers.append(
            Convolution(self.params["W6"], self.params["b6"],
    ...
    def accuracy(self, x, t, batch_size=100):
        ...
        for i in range(x.shape[0] // batch_size):
            ...
            y_batch = self.predict(x_batch, train_flag=False)
            y_batch = np.argmax(y_batch, axis=1)

            acc += np.sum(y_batch == t_batch)
    ...
    def save_params(self, file_name="./weight/vgg_params.pkl"):
        params = {}
    ...
    def load_params(self, file_name="./weight/vgg_params.pkl"):
        ...
        for i, layer_idx in enumerate((0, 2, 5, 7, 10, 12, 15, 18)):
            self.layers[layer_idx].dW = self.params[f"W{i+1}"]
            self.layers[layer_idx].db = self.params[f"b{i+1}"]
